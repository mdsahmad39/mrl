...
function AddEdit({ title, partyCollection }: { title: string, partyCollection?: any }) {
    ...
    const fields = {
        collectionCode: register('collectionCode'),
    ...
    async function onSubmit(data: any) {
        ...
        try {
            let message;
            const senderParty = parties?.find(p => p.partyCode === data?.senderPartyCode);
            const receiverParty = parties?.find(p => p.partyCode === data?.receiverPartyCode);
            const paymentParty = parties?.find(p => p.partyCode === data?.paymentPartyCode);
            const finalData = {
                ...data,
                senderParty: {
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className='flex flex-wrap gap-10 justify-evenly'>
                    ...
                    <div className="w-full sm:w-1/1 md:w-1/2 lg:w-1/3 xl:w-1/4 mb-4">
                        ...
                        <select {...fields.senderPartyCode} className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ${errors.senderPartyCode ? 'is-invalid' : ''}`} name="senderPartyCode" required
                            // onChange={(e) => {
                    ...
                    <div className="w-full sm:w-1/1 md:w-1/2 lg:w-1/3 xl:w-1/4 mb-4">
                        ...
                        <select {...fields.receiverPartyCode} className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ${errors.receiverPartyCode ? 'is-invalid' : ''}`} name="receiverPartyCode" required
                            // onChange={(e) => {
                    ...
                    <div className="w-full sm:w-1/1 md:w-1/2 lg:w-1/3 xl:w-1/4 mb-4">
                        ...
                        <select {...fields.paymentPartyCode} className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ${errors.paymentPartyCode ? 'is-invalid' : ''}`} name="paymentPartyCode" required
                            // onChange={(e) => {
                ...
                <div className="flex justify-center">
                    <div className="w-full sm:w-1/4 md:w-1/6 lg:w-1/8 xl:w-1/12 mb-4"><button type="submit" disabled={formState.isSubmitting} className="backdrop bg-white bg-opacity-0 border border-gray px-3 py-1.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-200 focus:ring-opacity-40 hover:bg-opacity-50 text-lg">
                        {formState.isSubmitting && <span className="spinner-border spinner-border-sm me-1"></span>}
                        Save
                    </button></div>
                    <div className="w-full sm:w-1/4 md:w-1/6 lg:w-1/8 xl:w-1/12 mb-4"><button onClick={() => reset()} type="button" disabled={formState.isSubmitting} className="backdrop bg-white bg-opacity-0 border border-gray px-3 py-1.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-200 focus:ring-opacity-40 hover:bg-opacity-50 text-lg">Reset</button></div>
                    <div className="w-full sm:w-1/4 md:w-1/6 lg:w-1/8 xl:w-1/12 mb-4"><Link href="/partycollections" className="backdrop bg-white bg-opacity-0 border border-gray px-3 py-1.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-200 focus:ring-opacity-40 hover:bg-opacity-50 text-lg">Cancel</Link></div>
                </div>
            </form>
        </div>
    );
